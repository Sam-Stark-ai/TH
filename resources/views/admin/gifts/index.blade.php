              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header card-no-border text-end">
                    <div class="card-header-right-icon"><a class="btn btn-primary f-w-500" href="{{route('admin.gift.add')}}"><i class="fa fa-plus pe-2"></i>Add Gifts</a></div>
                  </div>
                          ...
                          <tbody>
                            ...
                            <tr class="product-removes">
                              ...
                              <td>{{ $loop->iteration }}</td>
                                <td>
                                <p class="c-o-light">{{$gift->title}}</p>
                              </td>

                              <td>
 <p class="c-o-light">{{$gift->rings_count}}</p>
                              </td>

                              <td>
 <p class="c-o-light">{{$gift->price}}</p>
                              </td>

                              <td>
                          <div class="product-action">

  <a class="square-white" href="{{ route('admin.gift.edit', $gift->id) }}">
    <svg>
  ...
  <form action="{{ route('admin.gift.destroy', $gift->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this gift?');" style="display:inline;">
    @csrf
    @method('DELETE')
